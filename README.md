...
The basic idea for this project was to add a substitution layer after some form of block cipher encryption had been performed, such as [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard), hence the name. This is not just any substitution layer, but a pirate substitution layer to abstract it more (and because its funny). AES-128, AES-192, and AES-256 with [Electronic Code Book](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Electronic_codebook_(ECB)) (ECB) and [Cipher Block Chaining](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Cipher_block_chaining_(CBC)) (CBC) as provided by [PyCryptodome](https://pypi.org/project/pycryptodome/) are the supported encryption algorithms and there is custom IV support for CBC. 

At a high level an [S-box](https://en.wikipedia.org/wiki/S-box) of sorts is made from shuffling a pirate wordset psuedorandomly using the same key used to encrypt with AES. The AES encrypted bytes are substituted out for pirate terms using this S-box making it "piratey". This project is really not supposed to be practical, but more educational (and to be silly) as plaintext encrypted into "piratetext" becomes very large very quick since every byte maps to a whole word or phrase.
# Local Setup
1. After cloning the repository locally, install the needed dependencies using the included `requirements.txt` file like so:

    ```
    pip install -r requirements.txt
    ```
2. Next to start the [Flask](https://flask.palletsprojects.com/en/stable/) powered API simply run ```main.py```. If using a Python virtual environment make sure to run ```main.py``` in the activated environment.
